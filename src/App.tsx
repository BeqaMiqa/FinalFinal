...
import LastPageComponent from "./Components/PageButtons";
...
function App() {
  const [url,seturl] = useState(`https://api2.myauto.ge/ka/products/`);
  ...
  // }, []);
  useEffect(() => {
    seturl(`https://api2.myauto.ge/ka/products/?Page=${page}`);
  }, [page]);
  
  useEffect(() => {
    fetch(`${url}?Page=${page}&${query}&Period=${period}&SortOrder=${sortOrder}`)
      .then((res) => res.json())
  ...
  return (
    ...
    >
      ...
      {/* ----------------------------------------------------------------------------- */}
      <LastPageComponent page={page} setPage={setPage} />
    </div>
  );
}
